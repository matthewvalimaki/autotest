...
Vagrant.configure(2) do |config|
  ...
  config.vm.provision "shell", inline: <<-SHELL
	...
	rm -rf sdk.tgz
	# update 
	# get platform-tools, sdk and an image to build our headless android emulator
	( sleep 5 && while [ 1 ]; do sleep 1; echo y; done ) | android-sdk-linux/tools/android update sdk --no-ui -s --filter 1,2,4,26,38
	...
	sudo cp /vagrant/selenium /etc/init.d/selenium
